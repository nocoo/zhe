...
describe("zhe-edge Worker — fetch handler", () => {
  ...
  describe("KV redirect", () => {
    ...
    it("sends correct analytics payload to /api/record-click", async () => {
      ...
      const analyticsCall = fetchMock.mock.calls.find((call: unknown[]) =>
        (call[0] as string).includes("/api/record-click"),
      );
    ...
    it("calls lookup API on KV miss and redirects on hit", async () => {
      ...
      const lookupCall = fetchMock.mock.calls.find((call: unknown[]) =>
        (call[0] as string).includes("/api/lookup"),
      );
    ...
    it("returns origin 404 page and writes tombstone on lookup API miss", async () => {
      const fetchMock = vi.fn().mockImplementation((url: string) => {
        ...
        return Promise.resolve(new Response("<html>404 page</html>", { status: 404 }));
      });
      globalThis.fetch = fetchMock;
      const env = makeEnv();
      env.LINKS_KV.get.mockResolvedValue(null);
...
describe("zhe-edge Worker — scheduled handler", () => {
  ...
  it("logs error when cleanup endpoint returns non-200", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("Cleanup cron failed (500)"));
  });
  ...
  it("does not throw when cleanup fetch fails (network error)", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith("Cleanup cron fetch error:", expect.any(Error));
  });
  ...
  it("logs error when sync-kv endpoint returns non-200", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("Sync-kv cron failed (503)"));
  });
  ...
  it("handles network error for sync-kv gracefully", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith("Sync-kv cron fetch error:", expect.any(Error));
  });
});
...
describe("zhe-edge Worker — D1 proxy handler", () => {
  ...
  describe("routing", () => {
    ...
    it("forwards other /api/* paths to origin", async () => {
      ...
      const res = await worker.fetch(makeRequest("/api/health"), env, makeCtx());
    ...
    it("handles /api/d1-batch instead of forwarding to origin", async () => {
      ...
      env.DB.batch = vi
        .fn()
        .mockResolvedValue([{ results: [{ id: 1 }], meta: { changes: 1, last_row_id: 1 } }]);
