    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.name === "AbortError") {
          throw new ApiClientError(0, "Request timed out. Check your connection and try again.");
        }
        if (error.message.includes("ENOTFOUND") || error.message.includes("getaddrinfo")) {
          throw new ApiClientError(0, "Could not reach zhe.to. Check your connection.");
        }
      }
