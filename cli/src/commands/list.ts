...
import { formatLinksMinimal, formatLinksTable, resolveTagName } from "../utils.js";
...
async function loadDisplayMaps(
  ...
  const needsTagFallback =
    showTags && links.some((l) => (l.tagIds ?? []).length > 0 && (!l.tags || l.tags.length === 0));
  if (needsTagFallback) {
    try {
 ...
 */
function resolveFolderNameFromCache(input: string, folders: Folder[]): string | null {
