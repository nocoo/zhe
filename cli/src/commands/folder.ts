...
const createSubcommand = defineCommand({
  ...
  async run({ args }) {
    ...
    try {
      ...
      console.log(pc.green(`✓ Created folder ${pc.cyan(folder.name)} ${pc.dim(`(${folder.id})`)}`));
    } catch (error) {
...
const updateSubcommand = defineCommand({
  ...
  async run({ args }) {
    ...
    if (Object.keys(request).length === 0) {
      console.log(pc.yellow("No changes specified. Pass --name and/or --icon."));
      process.exit(EXIT_INVALID_ARGS);
    }
    ...
    try {
      ...
      const { folder } = response;
      console.log(pc.green(`✓ Updated folder ${pc.cyan(folder.name)} ${pc.dim(`(${folder.id})`)}`));
    } catch (error) {
      handleApiError(error);
    }
  },
