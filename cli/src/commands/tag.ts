...
function normalizeColor(input: string): string | null {
  if (!HEX_COLOR_PATTERN.test(input)) {
    console.log(pc.red("--color must be a 6-digit hex color (e.g. '#3b82f6' or 'ff5500')."));
    return null;
  }
...
const createSubcommand = defineCommand({
  ...
  async run({ args }) {
    ...
    try {
      ...
      const { tag } = response;
      console.log(
        pc.green(`✓ Created tag ${pc.cyan(tag.name)} ${pc.dim(`(${tag.id})`)} ${tag.color}`),
      );
    } catch (error) {
      handleApiError(error);
    }
...
const updateSubcommand = defineCommand({
  ...
  async run({ args }) {
    ...
    if (Object.keys(request).length === 0) {
      console.log(pc.yellow("No changes specified. Pass --name and/or --color."));
      process.exit(EXIT_INVALID_ARGS);
    }
    ...
    try {
      ...
      if (resolved.kind === "ambiguous") {
        console.log(
          pc.red(`Multiple tags match "${ref}". Please use the tag ID or rename duplicates.`),
        );
        process.exit(EXIT_INVALID_ARGS);
      }
      ...
      const { tag } = response;
      console.log(
        pc.green(`✓ Updated tag ${pc.cyan(tag.name)} ${pc.dim(`(${tag.id})`)} ${tag.color}`),
      );
    } catch (error) {
      handleApiError(error);
    }
    ...
    try {
      ...
      if (!args.yes) {
        if (!process.stdin.isTTY) {
          console.log(pc.red("Refusing to delete without --yes in non-interactive mode."));
          process.exit(EXIT_INVALID_ARGS);
        }
      ...
      if (args.json) {
        console.log(JSON.stringify({ success: true, id: tagId, name: tagName ?? null }, null, 2));
        return;
      }

      console.log(pc.green(`✓ Deleted tag ${pc.cyan(tagLabel)} ${pc.dim(`(${tagId})`)}`));
    } catch (error) {
      handleApiError(error);
    }
  },
