...
export function formatLinksTable(links: Link[], options: FormatLinksTableOptions = {}): string {
...
export function formatFoldersTable(folders: Folder[]): string {
  ...
  const header = "ID                                   NAME               LINKS  ICON     CREATED";
  const separator = "─".repeat(header.length);
...
export function formatTagsTable(tags: Tag[]): string {
  ...
  const header = "ID                                   NAME               COLOR    CREATED";
  const separator = "─".repeat(header.length);
