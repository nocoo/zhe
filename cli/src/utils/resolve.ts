 ...
 */
export async function resolveFolderName(client: ApiClient, input: string): Promise<string | null> {
  if (UUID_PATTERN.test(input)) {
    return input; // Assume it's already an ID
  }

  const { folders } = await client.listFolders();
  const matches = folders.filter((f: Folder) => f.name.toLowerCase() === input.toLowerCase());
  ...
  if (matches.length > 1) {
    console.log(pc.red(`Multiple folders match "${input}". Please use the folder ID instead.`));
 ...
 */
export async function resolveTagName(client: ApiClient, input: string): Promise<string | null> {
  if (UUID_PATTERN.test(input)) {
    return input;
  }

  const { tags } = await client.listTags();
  const matches = tags.filter((t: Tag) => t.name.toLowerCase() === input.toLowerCase());
  ...
  if (matches.length > 1) {
    console.log(
      pc.red(`Multiple tags match "${input}". Please use the tag ID or rename duplicates.`),
    );
    return null;
  }
...
export async function resolveTagRef(client: ApiClient, input: string): Promise<TagRef> {
  if (UUID_PATTERN.test(input)) {
