...
- **Preset configs lock you to the slowest transitive plugin**: `eslint-config-next` pulls in `eslint-plugin-react`, `eslint-plugin-jsx-a11y`, and `eslint-plugin-import` as transitive deps; when eslint 10 dropped `context.getFilename()` / `context.getSourceCode()`, those plugins all broke and the preset stayed unusable until jsx-eslint shipped a fix. For an infra upgrade with a clean cliff like this, **don't patch node_modules** (the patch becomes a maintenance burden and rots after the upstream fix lands wrong). Hand-roll the flat config directly from `@next/eslint-plugin-next` + `eslint-plugin-react-hooks` + `typescript-eslint` — the project is 100% TS/TSX so the babel parser the preset shipped is dead weight anyway. (2026-06-22: did this to unblock eslint 10. Net rule loss was ~7 jsx-a11y `warn`s the preset already down-rated, well worth shedding the transitive lock.)
