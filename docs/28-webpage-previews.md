...
列表和网格卡片的“删除截图”操作替代旧的截图来源选择。按钮仅在普通站点有截图时显示；GitHub 共享预览与 X 附件不提供该操作。服务端校验用户及原截图 URL，先持久记录自有 R2 对象的清理任务，再按原 URL 条件清空字段。现有触发器移除完成的截图任务，下一轮 Connector 自动重新发现该链接。旧版截图来源产生的 R2 文件同样清理；其他链接或上传记录仍引用的文件保留，R2 失败由清理队列重试。重复或延迟请求不会清除已重新生成的截图。
