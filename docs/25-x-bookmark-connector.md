...
「全部链接」中的 X 帖子使用与其他链接相同的紧凑卡片：网格封面固定为 4:3，正文显示摘要，视频显示海报。补全结果会自动更新摘要和状态，不会将卡片撑成长帖。点击预览或标题，在详情弹窗中阅读正文、查看图片和播放视频；关闭弹窗会停止播放。列表视图同样保留紧凑布局。

侧边栏的「X 收藏」（`/dashboard/x`）汇集所有文件夹的 X 帖子，以较宽的帖子卡片直接展示正文和媒体。支持分类（含 Inbox）、正文／作者／备注搜索，以及视频、图片、GIF、文章、文字和待补全筛选。分类、搜索与内容类型取交集；同一个混合媒体帖子可以同时匹配视频和图片。
