...
侧边栏的「X 收藏」（`/dashboard/x`）汇集所有文件夹的 X 帖子。卡片正文使用与其他页面一致的 14px 字号和 20px 行高，默认最多五行；媒体显示首个附件或最多四张图片的拼图，预览高度不超过 256px。分类、标签数量和操作图标合为一行 footer，完整正文、全部附件、互动统计及补全重试均在详情弹窗查看。支持分类（含 Inbox）、正文／作者／备注搜索，以及视频、图片、GIF、文章、文字和待补全筛选。分类、搜索与内容类型取交集；同一个混合媒体帖子可以同时匹配视频和图片。
