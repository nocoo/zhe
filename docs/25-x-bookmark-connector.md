...
可选的 [Eagle 图片旁路](26-eagle-sidecar.md) 默认关闭：配置本机 GDrive Eagle library 后，Connector 对 1–4 张图片独立并行归档，失败不影响 Zhe 补全。

「全部链接」中的 X 帖子使用与其他链接相同的紧凑卡片：网格封面固定为 4:3，正文显示摘要，视频显示海报。补全结果自动更新摘要和右侧详情图标的状态提示，不增加状态行。点击预览、标题或详情图标，在弹窗中阅读正文、查看图片和播放视频；关闭弹窗会停止播放。列表视图同样保留紧凑布局，网格的标签并入原有元信息行。

侧边栏的「X 收藏」（`/dashboard/x`）汇集所有文件夹的 X 帖子。卡片正文使用与其他页面一致的 14px 字号和 20px 行高，默认最多五行；媒体显示首个附件或最多四张图片的拼图，预览高度不超过 256px。Footer 显示分类、标签名称和操作图标，完整正文、全部附件、互动统计及补全重试均在详情弹窗查看。支持分类（含 Inbox）、标签、正文／作者／备注搜索，以及视频、图片、GIF、文章、文字和待补全筛选。分类、标签、搜索与内容类型取交集；同一个混合媒体帖子可以同时匹配视频和图片。

书签区的「特殊来源」筛选默认包含 GitHub、排除 X，用户可自行修改；独立 X 页面不受开关影响。GitHub 采集、仓库展示与 Inbox 规则见[特殊来源与 GitHub 收藏](27-github-bookmarks.md)。

媒体类型按当前可用的归档附件识别。文章目前包含长文（正文超过 600 个字符）和带展开外链或 X Article 链接的帖子；现有归档数据没有原生文章类型标记。待补全包含尚未读取、处理中、失败和媒体未完成的帖子。浏览页复用共享补全状态，自动更新筛选结果和附件。
