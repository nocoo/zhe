...
可在配置文件的 `eagle` 对象设置 `timeoutMs`（默认 180000，范围 1000–600000）和 `retryMs`（默认 5000，范围 1000–300000）。已入队任务保留当时的超时和目标库设置。切换库路径时，旧库任务记录 `paused_library`，不会被悄悄改写到新库；切回原路径并重启即可继续。关闭功能会暂停全部旁路恢复。
...
- 每图单独下载、校验、生成缩略图、重试。每个 attempt 运行于独立进程组，共享一个总时限（默认 180 秒），下载与缩略图阶段为后续写入预留时间。超时终止 Python、Node 和 FFmpeg，并最多用 250ms 排空已写出的完成结果；无法确认完成时才记录 timeout。Drive 文件操作全部位于子进程中，避免阻塞 enrich 的文件操作线程。
