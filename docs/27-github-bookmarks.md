...
卡片根据内容区宽度显示 1–4 列，同一行卡片等高，不同行按各自内容确定高度。复用收藏卡片的圆角、阴影和底栏，统计与语言并排，去掉空内容的固定占位。标题 1 行，备注最多 2 行，原始简介弱化为 1 行；重复文字只显示一次，文字可展开查看。最多展示 3 个主题标签和 2 个收藏标签，多余标签显示数量，点击可查看完整内容。编辑收藏使用弹窗，不改变网格高度。
