...
describe("kvBulkPutLinks", () => {
  ...
  it("includes expiration field when expiresAt is in the future", async () => {
    ...
    const entries = [
      {
        slug: "no-exp",
        data: { id: 1, originalUrl: "https://a.com", expiresAt: null } as KVLinkData,
      },
      {
        slug: "has-exp",
        data: { id: 2, originalUrl: "https://b.com", expiresAt: futureMs } as KVLinkData,
      },
    ];
  ...
  it("omits expiration field when expiresAt is in the past", async () => {
    ...
    const entries = [
      {
        slug: "old",
        data: { id: 1, originalUrl: "https://old.com", expiresAt: pastMs } as KVLinkData,
      },
    ];
...
describe("kvListKeys", () => {
  ...
  it("returns error flag on non-ok response", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith("KV list keys failed:", 500, "Internal Server Error");
    consoleSpy.mockRestore();
  });
  ...
  it("returns error flag on unsuccessful response", async () => {
    ...
    expect(result).toEqual({ keys: [], error: true });
    expect(consoleSpy).toHaveBeenCalledWith("KV list keys returned unsuccessful response");
    consoleSpy.mockRestore();
  });
  ...
  it("returns error flag on network error", async () => {
    ...
    expect(result).toEqual({ keys: [], error: true });
    expect(consoleSpy).toHaveBeenCalledWith("KV list keys error:", expect.any(Error));
    consoleSpy.mockRestore();
  });
...
describe("kvBulkDeleteLinks", () => {
  ...
  it("counts failed entries on network error", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith("KV bulk delete error (batch 0):", expect.any(Error));
    consoleSpy.mockRestore();
  });
});
