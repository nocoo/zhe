...
describe("models/links", () => {
  ...
  describe("buildShortUrl", () => {
    ...
    it("handles trailing slash-free base url", () => {
      expect(buildShortUrl("http://localhost:3000", "test")).toBe("http://localhost:3000/test");
    });
  });
  ...
  describe("hasAnalyticsData", () => {
    ...
    it("returns true when uniqueCountries is non-empty", () => {
      expect(hasAnalyticsData({ ...emptyStats, uniqueCountries: ["US"] })).toBe(true);
    });
  ...
  describe("filterLinks", () => {
    ...
    it("matches by note substring", () => {
      const richLinks = [
        makeLink({
          ...
          slug: "a",
          originalUrl: "https://a.com",
          note: "Important reference for project",
        }),
    ...
    it("does not match tag names when context is not provided", () => {
      const taggedLinks = [makeLink({ id: 10, slug: "a", originalUrl: "https://a.com" })];
    ...
    it("matches link with tag even if other fields do not match", () => {
      const taggedLinks = [makeLink({ id: 10, slug: "x", originalUrl: "https://x.com" })];
      const ctx = {
        tags: [{ id: "t1", userId: "u1", name: "Design", color: "#00ff00", createdAt: new Date() }],
        linkTags: [{ linkId: 10, tagId: "t1" }],
      };
    ...
    it("matches Chinese characters in tag names", () => {
      const cnLinks = [makeLink({ id: 1, slug: "a", originalUrl: "https://a.com" })];
      const ctx = {
        tags: [
    ...
    it("ignores linkTag entries referencing nonexistent tagId", () => {
      const taggedLinks = [makeLink({ id: 10, slug: "x", originalUrl: "https://x.com" })];
      const ctx = {
        tags: [{ id: "t1", userId: "u1", name: "Real", color: "#ff0000", createdAt: new Date() }],
        linkTags: [
          { linkId: 10, tagId: "t1" },
    ...
    it("does not crash when linkTag references nonexistent linkId", () => {
      const taggedLinks = [makeLink({ id: 10, slug: "x", originalUrl: "https://x.com" })];
      const ctx = {
        tags: [{ id: "t1", userId: "u1", name: "Orphan", color: "#ff0000", createdAt: new Date() }],
        linkTags: [
    ...
    it("returns multiple links when each matches via different fields", () => {
      const mixedLinks = [
        ...
        makeLink({
          ...
          metaDescription: "alpha reference",
        }),
        makeLink({
          id: 3,
          slug: "gamma",
          originalUrl: "https://three.com",
          note: "see alpha docs",
        }),
      ];
  ...
  describe("highlightMatches", () => {
    ...
    it("returns non-highlighted when no match found", () => {
      expect(highlightMatches("hello", "xyz")).toEqual([{ text: "hello", highlight: false }]);
    });

    it("handles entire string as match", () => {
      expect(highlightMatches("abc", "abc")).toEqual([{ text: "abc", highlight: true }]);
    });
  ...
  describe("buildLinkCounts", () => {
    ...
    it("handles all links uncategorized", () => {
      const links = [makeLink({ id: 1, folderId: null }), makeLink({ id: 2, folderId: null })];
      const counts = buildLinkCounts(links);
      expect(counts.total).toBe(2);
  ...
  describe("fetchScreenshotDomains", () => {
    ...
    it("returns the final redirected URL on success", async () => {
      ...
      const fetchSpy = vi.spyOn(globalThis, "fetch").mockResolvedValue(mockResponse(200, finalUrl));
