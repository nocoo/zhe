...
describe("getTodos", () => {
  it("returns a flat list sorted by (parent_id, position) with tag names populated", async () => {
    ...
    expect(
      head(
        list.filter((n) => n.id === a),
        "child a",
      ).tagNames,
    ).toEqual(["urgent", "work"]);
    expect(
      head(
        list.filter((n) => n.id === b),
        "child b",
      ).tagNames,
    ).toEqual(["home"]);
  });
...
describe("createTodo", () => {
  it("rejects an empty title", async () => {
    await expect(createTodo("u1", { title: "   " })).rejects.toThrow(/title cannot be empty/i);
  });

  it("rejects a missing parent", async () => {
    await expect(createTodo("u1", { title: "child", parentId: 999 })).rejects.toBeInstanceOf(
      TodoNotFoundError,
    );
  });
  ...
  it("rejects when the resulting depth would exceed the cap", async () => {
    ...
    await expect(createTodo("u1", { title: "overflow", parentId: deepest })).rejects.toBeInstanceOf(
      TodoDepthExceededError,
    );
  });
  ...
  it("rolls back the todo insert when tag insert fails", async () => {
    ...
    const spy = vi.spyOn(clientMod, "executeD1Batch").mockRejectedValueOnce(new Error("boom"));
    await expect(createTodo("u1", { title: "bad", tagNames: ["x"] })).rejects.toThrow(/boom/);
    spy.mockRestore();
...
describe("updateTodo", () => {
  ...
  it("flips done/doneAt in lockstep on state change and leaves doneAt intact on a no-op", async () => {
    ...
    if (!first?.doneAt) throw new Error("first update missing doneAt");
...
describe("moveTodo — reparent", () => {
  ...
  it("rejects a cycle via the write-time guard when moving an ancestor under its own descendant", async () => {
    ...
    await expect(moveTodo("u1", g, { parentId: c, position: 0 })).rejects.toBeInstanceOf(
      TodoMoveConflictError,
    );
  ...
  it("rejects cross-user parent — write-time EXISTS(newParent) predicate fails", async () => {
    ...
    await expect(moveTodo("u1", mine, { parentId: theirs, position: 0 })).rejects.toBeInstanceOf(
      TodoMoveConflictError,
    );
  });
  ...
  it("parent-deleted mid-move: guarded UPDATE's EXISTS predicate fails cleanly", async () => {
    ...
    await expect(moveTodo("u1", src, { parentId: dst, position: 0 })).rejects.toBeInstanceOf(
      TodoMoveConflictError,
    );
  ...
  it("Phase 3 gap: if the moving row is tampered with between Phase 2 and Phase 3, no reconcile writes leak", async () => {
    ...
    const spy = vi.spyOn(clientMod, "executeD1Batch").mockImplementationOnce(async (statements) => {
    ...
    await expect(moveTodo("u1", b, { parentId: other, position: 0 })).rejects.toBeInstanceOf(
      TodoMoveConflictError,
    );
    spy.mockRestore();
...
describe("reorderSiblings", () => {
  ...
  it("rejects an id set that includes a stranger (wrong user or wrong parent)", async () => {
    ...
    await expect(reorderSiblings("u1", p1, [a, b])).rejects.toBeInstanceOf(TodoMoveConflictError);
  });
  ...
  it("rejects a partial orderedIds set and leaves every sibling position untouched", async () => {
    ...
    const after = await snapshot("u1");
    expect([a, b, c].map((id) => after.byId(id).position)).toEqual(positionsBefore);
  });
  ...
  it("rejects duplicate ids in orderedIds", async () => {
    ...
    const a = await makeChild("u1", root, "a");
    const b = await makeChild("u1", root, "b");
    await expect(reorderSiblings("u1", root, [a, a, b])).rejects.toBeInstanceOf(
      TodoMoveConflictError,
    );
  });
