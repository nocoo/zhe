...
describe("models/settings", () => {
  describe("serializeLinksForExport", () => {
    ...
    it("exports null for missing optional fields", () => {
      const link = makeLink();
      const result = serializeLinksForExport([link]);
      const exported = unwrap(result[0]);
    ...
    it("serializes multiple links preserving order", () => {
      const links = [makeLink({ id: 1, slug: "first" }), makeLink({ id: 2, slug: "second" })];
    ...
    it("handles null clicks as 0", () => {
      ...
      const result = serializeLinksForExport([link]);
      expect(unwrap(result[0]).clicks).toBe(0);
    });
  });
  ...
  describe("parseImportPayload", () => {
    ...
    it("parses valid exported links", () => {
      ...
      expect(unwrap(unwrap(result.data)[0]).originalUrl).toBe("https://example.com");
      expect(unwrap(unwrap(result.data)[0]).slug).toBe("abc");
    });
    ...
    it("returns error when link missing slug", () => {
      const payload = [
        { originalUrl: "https://example.com", isCustom: false, clicks: 0, createdAt: "2026-01-01" },
      ];
      const result = parseImportPayload(payload);
      expect(result.success).toBe(false);
    ...
    it("defaults clicks to 0 when missing", () => {
      ...
      const result = parseImportPayload(payload);
      expect(result.success).toBe(true);
      expect(unwrap(unwrap(result.data)[0]).clicks).toBe(0);
    });

    it("defaults isCustom to false when missing", () => {
      const payload = [
        { originalUrl: "https://example.com", slug: "abc", clicks: 5, createdAt: "2026-01-01" },
      ];
      const result = parseImportPayload(payload);
      expect(result.success).toBe(true);
      expect(unwrap(unwrap(result.data)[0]).isCustom).toBe(false);
    });
    ...
    it("defaults optional fields to null when missing", () => {
      ...
      const result = parseImportPayload(payload);
      expect(result.success).toBe(true);
      const entry = unwrap(unwrap(result.data)[0]);
