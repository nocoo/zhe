...
const {
  ...
  return {
    mockSend,
    mockS3Client: vi.fn().mockImplementation(function () {
...
describe("R2 Client", () => {
  ...
  describe("getR2Config (indirect)", () => {
    ...
    it("throws when all R2 env vars are missing", async () => {
      await expect(deleteR2Object("key.png")).rejects.toThrow("Missing R2 configuration");
    });
  });
  ...
  describe("uploadBufferToR2", () => {
    ...
    it("throws when env vars are missing", async () => {
      ...
      await expect(uploadBufferToR2("key.png", body, "image/png")).rejects.toThrow(
        "Missing R2 configuration",
      );
    });
  });
  ...
  describe("deleteR2Objects", () => {
    ...
    it("throws when env vars are missing", async () => {
      await expect(deleteR2Objects(["key.png"])).rejects.toThrow("Missing R2 configuration");
    });
  });
