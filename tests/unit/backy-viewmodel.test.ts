...
describe('useBackyViewModel', () => {
  ...
  // Clear results
  // ==================================================================
  it('clearTestResult clears test result', async () => {
    mockGetBackyConfig.mockResolvedValue({
      success: true,
      data: { webhookUrl: 'https://backy.example.com/webhook', maskedApiKey: 'sk-1••••cdef' },
    });
  ...
  it('clearPushResult clears push result', async () => {
    mockGetBackyConfig.mockResolvedValue({
      success: true,
      data: { webhookUrl: 'https://backy.example.com/webhook', maskedApiKey: 'sk-1••••cdef' },
    });
    ...
    expect(result.current.pushResult).not.toBeNull();

    act(() => {
      result.current.clearPushResult();
    });
    expect(result.current.pushResult).toBeNull();
  });
});
