...
vi.mock("@/lib/db/scoped", async () => {
  ...
  const actual = await vi.importActual<typeof import("@/lib/db/scoped")>("@/lib/db/scoped");
  return {
    ...actual,
...
import { TodoDepthExceededError, TodoMoveConflictError, TodoNotFoundError } from "@/lib/db/scoped";
...
describe("createTodo", () => {
  ...
  it("rejects a non-integer parentId without touching the DB", async () => {
    expect(await createTodo({ title: "t", parentId: 1.5 as unknown as number })).toEqual({
      success: false,
...
describe("moveTodo", () => {
  ...
  it("rejects a non-integer parentId", async () => {
    expect(await moveTodo(1, { parentId: 1.5 as unknown as number, position: 0 })).toEqual({
      success: false,
      error: "Parent id must be an integer or null",
    });
    expect(mockMoveTodo).not.toHaveBeenCalled();
  ...
  it("forwards a valid move and returns the affected slice", async () => {
    ...
    expect(await moveTodo(1, { parentId: 2, position: 0 })).toEqual({ success: true, data: slice });
  ...
  it("never throws — an unknown error still returns a generic failure envelope", async () => {
    ...
    await expect(moveTodo(1, { parentId: 2, position: 0 })).resolves.toEqual({
      success: false,
...
describe("reorderTodoSiblings", () => {
  ...
  it("rejects a non-integer id in the list without touching the DB", async () => {
    expect(await reorderTodoSiblings(null, [1, 2.5 as unknown as number])).toEqual({
      success: false,
      error: "orderedIds must be integers",
    });
