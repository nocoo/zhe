...
if (typeof navigator !== "undefined") {
  try {
    ...
    /* jsdom path: already writable, ignore */
  }
}
...
vi.mock("@/lib/db/d1-client", async () => {
  ...
  const queryFn = async <T>(sql: string, params: unknown[] = []): Promise<T[]> => {
    ...
    if (
      sqlLower.includes("count(1)") &&
      sqlLower.includes("from links") &&
      sqlLower.includes("where slug = ?")
    ) {
      const [slug] = params;
      let cnt = 0;
    ...
    if (
      ...
      sqlLower.includes("and original_url = ?")
    ) {
    ...
    if (
      ...
      for (const link of mockLinks.values()) {
        ...
        if (rawLink.user_id === userId) {
          results.push({
            slug: rawLink.slug,
            original_url: rawLink.original_url,
            clicks: (rawLink.clicks as number) ?? 0,
    ...
    if (
      sqlLower.includes("from links") &&
      sqlLower.includes("where user_id = ?") &&
      sqlLower.includes("order by clicks desc")
    ) {
      const [userId] = params;
      const results: unknown[] = [];
      for (const link of mockLinks.values()) {
      ...
      results.sort(
        (a, b) =>
          ((b as Record<string, unknown>).clicks as number) -
          ((a as Record<string, unknown>).clicks as number),
      );
      return results as T[];
    }
    ...
    if (
      sqlLower.includes("count(distinct l.id)") &&
      sqlLower.includes("as cnt") &&
      sqlLower.includes("from links")
    ) {
    ...
    if (
      sqlLower.startsWith("select") &&
      sqlLower.includes("from links") &&
      (sqlLower.includes("where user_id = ?") || sqlLower.includes("where l.user_id = ?"))
    ) {
    ...
    if (
      sqlLower.includes("update links set clicks = clicks + 1") &&
    ...
    if (
      sqlLower.includes("count(*)") &&
      sqlLower.includes("from analytics") &&
      sqlLower.includes("join links") &&
      sqlLower.includes("group by date") &&
      !sqlLower.includes("a.link_id = ?")
    ) {
      const [userId] = params;
      ...
      const dateCounts: Record<
        string,
        { clicks: number; origin_clicks: number; worker_clicks: number }
      > = {};
      for (const a of mockAnalytics) {
        ...
        if (userLinkIds.has(rawA.link_id as number)) {
          ...
          if (!dateCounts[date])
            dateCounts[date] = { clicks: 0, origin_clicks: 0, worker_clicks: 0 };
      ...
      return Object.entries(dateCounts)
        ...
        .map(([date, counts]) => ({
          ...
          origin_clicks: counts.origin_clicks,
          worker_clicks: counts.worker_clicks,
        })) as T[];
    }
    ...
    if (
      sqlLower.includes("select count(*)") &&
      sqlLower.includes("from analytics") &&
      sqlLower.includes("join links") &&
      sqlLower.includes("a.link_id = ?")
    ) {
      ...
      const count = mockAnalytics.filter(
        (a) => (a as unknown as Record<string, unknown>).link_id === linkId,
      ).length;
    ...
    if (
      sqlLower.includes("select distinct") &&
    ...
    if (
      sqlLower.includes("select count(*)") &&
      sqlLower.includes("from analytics") &&
      sqlLower.includes("where link_id = ?") &&
      !sqlLower.includes("join links")
    ) {
      const [linkId] = params;
    ...
    if (
      sqlLower.includes("count(*)") &&
      sqlLower.includes("total_uploads") &&
      sqlLower.includes("from uploads") &&
      sqlLower.includes("where user_id = ?")
    ) {
      const [userId] = params;
      let totalUploads = 0;
    ...
    if (
      sqlLower.includes("count(*)") &&
      sqlLower.includes("file_type") &&
      sqlLower.includes("from uploads") &&
      sqlLower.includes("where user_id = ?") &&
      sqlLower.includes("group by file_type")
    ) {
      const [userId] = params;
    ...
    if (
      sqlLower.includes("from folders f") &&
      sqlLower.includes("left join links") &&
      sqlLower.includes("count(l.id)")
    ) {
      const [userId] = params;
      const results: unknown[] = [];
      for (const folder of mockFolders.values()) {
    ...
    if (
      sqlLower.includes("from tags t") &&
      sqlLower.includes("join link_tags") &&
      sqlLower.includes("link_id = ?")
    ) {
      const [linkId, userId] = params;
      const linkTags = getMockLinkTags();
      const mockTags = getMockTags();
    ...
    if (
      sqlLower.includes("from link_tags") &&
      sqlLower.includes("join links") &&
      sqlLower.includes("user_id")
    ) {
      const [userId] = params;
    ...
    if (
      sqlLower.startsWith("delete from link_tags") &&
      sqlLower.includes("where link_id = ?") &&
      sqlLower.includes("tag_id = ?")
    ) {
    ...
    if (
      sqlLower.startsWith("select") &&
      sqlLower.includes("from webhooks") &&
      sqlLower.includes("where user_id = ?")
    ) {
      const [userId] = params;
    ...
    if (
      sqlLower.startsWith("delete from webhooks") &&
      sqlLower.includes("where user_id = ?") &&
    ...
    if (sqlLower.startsWith("insert into user_settings")) {
      ...
      if (
        sqlLower.includes("backy_pull_key") &&
        !sqlLower.includes("backy_webhook_url") &&
        !sqlLower.includes("xray_api_url")
      ) {
    ...
    if (
      sqlLower.includes("from tweet_cache") &&
      sqlLower.includes("where tweet_id = ?") &&
    ...
    if (
      sqlLower.includes("from api_keys") &&
      sqlLower.includes("where user_id = ?") &&
      sqlLower.includes("revoked_at is null")
    ) {
      const [userId] = params;
      const mockKeys = getMockApiKeys();
      const results: unknown[] = [];
    ...
    if (
      sqlLower.includes("update api_keys") &&
      sqlLower.includes("set revoked_at = ?") &&
      sqlLower.includes("where id = ?") &&
      sqlLower.includes("user_id = ?")
    ) {
    ...
    if (
      sqlLower.includes("update api_keys") &&
      sqlLower.includes("set last_used_at = ?") &&
      sqlLower.includes("where id = ?")
    ) {
    ...
    if (
      sqlLower.includes("count(distinct i.id)") &&
      sqlLower.includes("as cnt") &&
      sqlLower.includes("from ideas")
    ) {
      const mockIdeas = getMockIdeas();
