 ...
 *   - Pull API route (/api/backy/pull — POST + HEAD)
...
describe('Backy backup integration (E2E)', () => {
  ...
  // =========================================================================
  // 7. Pull API route (/api/backy/pull)
  // =========================================================================
  describe('pull API route', () => {
    ...
    it('POST pushes backup to Backy and returns 200 with stats', async () => {
      ...
      // Seed link data
      await seedLink('https://example.com/pull-test', {
  ...
  describe('multi-user isolation', () => {
    ...
    it('HEAD /api/backy/pull rejects key from wrong user context', async () => {
      authenticatedAs(USER_A);
      const { generateBackyPullWebhook } = await import('@/actions/backy');
