...
test.describe
  .serial("Backy Page", () => {
    ...
    test("saves push config and shows configured state", async ({ page }) => {
      ...
      // Fill config form
      await page
        .locator('[data-testid="backy-webhook-url"]')
        .fill("https://backy.example.com/api/webhook/test");
      ...
      await expect(page.locator("code").filter({ hasText: "backy.example.com" })).toBeVisible({
        timeout: 10_000,
      });
    ...
    test("edit button shows form with current values, cancel returns to display", async ({
      page,
    }) => {
      await goToBacky(page);
    ...
    test("generates pull webhook credentials", async ({ page }) => {
      ...
      await expect(page.locator("code").filter({ hasText: "/api/backy/pull" }).first()).toBeVisible(
        { timeout: 10_000 },
      );
    ...
    test("regenerates pull webhook key with a new value", async ({ page }) => {
      ...
      const pullKeyCode = page
        .locator("code.font-mono")
        .filter({ hasNotText: "X-Webhook-Key" })
        .filter({ hasNotText: "curl" });
