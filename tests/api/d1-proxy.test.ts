...
describe("POST /api/d1-query — SQL execution", () => {
  it("executes SELECT query and returns results", async () => {
    // Insert test data
    await proxyQuery(`INSERT INTO ${TEST_TABLE} (user_id, value, created_at) VALUES (?, ?, ?)`, [
      TEST_USER_ID,
      "test-select",
      Date.now(),
    ]);
...
describe("POST /api/d1-query — error handling", () => {
  it("normalizes UNIQUE constraint errors (HTTP 200, stripped message)", async () => {
    ...
    await proxyQuery(
      `CREATE UNIQUE INDEX IF NOT EXISTS idx_proxy_test_user_value ON ${TEST_TABLE}(user_id, value)`,
    );
  ...
  it("sanitizes FOREIGN KEY errors to generic message (HTTP 200)", async () => {
    ...
    const { status, body } = await proxyQuery(`INSERT INTO nonexistent_table (id) VALUES (1)`);
